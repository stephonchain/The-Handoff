...
//  ShiftMood
//
//  Complete SwiftData models for iOS native app
...
final class Mood {
    ...
    // Pre-shift metrics (always present)
    var energy: Int // 1-5
    var stress: Int // 1-5
    var motivation: Int // 1-5
    
    // Post-shift metrics (optional, only for post-mood)
    var emotionalLoad: Int? // 1-5
    var fatigue: Int? // 1-5
    var satisfaction: Int? // 1-5
    
    // Emoji badges (unicode strings)
    var badges: [String]
    ...
    // Convenience initializer for post-shift mood
    init(fatigue: Int, emotionalLoad: Int, satisfaction: Int, badges: [String] = []) {
        self.id = UUID()
        self.timestamp = Date()
        self.energy = 0 // Not used in post-shift
        self.stress = 0 // Not used in post-shift
        self.motivation = 0 // Not used in post-shift
...
final class Vacation {
    ...
    var customName: String? // Optional custom name
    ...
    var daysCount: Int {
        ...
        return (components.day ?? 0) + 1 // Include end date
    }
...
final class DailyAffirmation {
    ...
    var shownDate: Date?
    var liked: Bool
    var sourceIndex: Int? // Reference to original affirmation in static list
...
extension Mood {
    static var previewPreShift: Mood {
        return Mood(energy: 4, stress: 2, motivation: 5)
    }
    
    static var previewPostShift: Mood {
        let mood = Mood(fatigue: 4, emotionalLoad: 3, satisfaction: 4, badges: ["😊", "💪"])
        return mood
    }
}
...
extension JournalEntry {
    static var preview: JournalEntry {
        let entry = JournalEntry(
            ...
            content: "So many patients tonight, little sleep. But I managed to stabilize Mr. Johnson, that's a win. Learned a new technique with Sophie.",
            moodEmoji: "😴"
        )
        entry.tags = ["Night", "Fatigue", "Pride", "Learning"]
        entry.highlights = [
    ...
    static var previewShort: JournalEntry {
        return JournalEntry(
            title: "Great day",
            content: "Touching encounter with a patient who thanked me.",
            moodEmoji: "😊"
        )
    }
}
...
extension Vacation {
    ...
    static var previewPast: Vacation {
        let startDate = Calendar.current.date(byAdding: .day, value: -30, to: Date())!
        let endDate = Calendar.current.date(byAdding: .day, value: -23, to: Date())!
        return Vacation(startDate: startDate, endDate: endDate, type: .compTime, status: .completed)
    }
}
